...
* `connection_bots_used` (array of strings): Declare what bots this bot will access through the [bot connection API](../../api-to-access-bots-on-poe). The default is an empty list. The entries in the list may be handles of Poe bots or the string "any", indicating that the bot may invoke any other bot. For example, setting this field to `["chatgpt"]` declares that the bot will use only [ChatGPT](https://poe.com/ChatGPT), and setting it to `["any"]` declares that the bot may use any other bot. Poe will show this value to users, and will enforce that bots do not access other bots that they did not declare beforehand.
